// Real Steam Partner Financial API wishlist fetcher (Milestone 3, moved to
// shared lib in M7).
//
...
/** Latest completed Steam reporting date. */
export function latestCompletedReportingDateIso(now: Date = new Date()): string {
  const y = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - 1));
  return y.toISOString().slice(0, 10);
}

/** Inclusive 7-day window ending on the latest completed reporting date. */
