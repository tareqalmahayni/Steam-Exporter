 ...
 *
 * M7 update: a `parseTrafficCsvText(text, fileName)` overload accepts the CSV
 * text directly so the API server can parse uploads without writing them to
 * disk first.
 */
...
/** Parse a CSV from text content (M7 — used by the API server for uploads). */
export function parseTrafficCsvText(text: string): TrafficCsvParseResult {
  ...
  for (let i = 1; i < grid.length; i++) {
    ...
    const pageFeature = line[fmt.columnIndexes["Page / Feature"]] ?? "";
    if (pageCategory === "") {
...
export function parseTrafficCsv(filePath: string): TrafficCsvParseResult {
  ...
  return parseTrafficCsvText(text);
}

const FILENAME_RE = /^traffic_(.+)_(\d+)_(\d{8})_(\d{8})\.csv$/;

export function parseTrafficFilename(filePathOrName: string): ParsedTrafficFilename {
...
export function validateTrafficFileIdentity(
  ...
  const errors: string[] = [];
  const parsed = parseTrafficFilename(filePathOrName);
  if (!parsed.ok) {
